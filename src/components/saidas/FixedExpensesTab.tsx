...
import {
  Plus, Trash2, AlertTriangle, CalendarClock, Loader2,
  CheckCircle2, Clock, FileText, RotateCcw,
} from "lucide-react";
...
const FixedExpensesTab = () => {
  ...
  const addMutation = useMutation({
    mutationFn: async (exp: { nome: string; valor: number; dia_vencimento: number; categoria: string }) => {
      ...
      const { error } = await supabase.from("fixed_expenses").insert({
        user_id: user.id, nome: exp.nome, valor: exp.valor,
        dia_vencimento: exp.dia_vencimento, categoria: exp.categoria, status: "pendente",
      });
      if (error) throw error;
    },
    onSuccess: () => { invalidateDashboardData(queryClient); toast.success("Conta recorrente adicionada!"); },
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { invalidateDashboardData(queryClient); toast.success("Conta removida definitivamente."); },
    onError: () => toast.error("Erro ao remover conta."),
  });

  const deleteOccurrenceMutation = useMutation({
    mutationFn: async (occurrenceId: string) => {
      const { error } = await supabase.from("fixed_expense_occurrences").delete().eq("id", occurrenceId);
      if (error) throw error;
    },
    onSuccess: () => { invalidateDashboardData(queryClient); toast.success("Removido apenas deste mês."); },
  ...
  const addFixedExpense = () => {
    if (!nome || !valor) { toast.error("Preencha todos os campos."); return; }
    addMutation.mutate({ nome, valor: parseFloat(valor), dia_vencimento: diaRecorrente, categoria });
    setNome(""); setValor(""); setDiaRecorrente(1); setCategoria(categorias[0]); setShowFixedForm(false);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => setShowFixedForm(!showFixedForm)}>
      ...
      {totals.total > 0 && (
        <div className="glass-card p-4">
          ...
          <p className="text-2xl font-bold font-mono text-foreground">{formatSimple(totals.total)}</p>
          ...
          <div className="w-full h-1.5 rounded-full bg-muted mt-3">
            <div className="h-full rounded-full bg-primary transition-all" style={{ width: `${totals.total > 0 ? (totals.paid / totals.total) * 100 : 0}%` }} />
          </div>
        </div>
      )}
        ...
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} className="glass-card p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={diaRecorrente} onChange={(e) => setDiaRecorrente(Number(e.target.value))} className="bg-secondary border border-border rounded-md px-3 py-2 text-sm w-full">
                {diasDoMes.map((d) => <option key={d} value={d}>Todo dia {d}</option>)}
              </select>
            </div>
        ...
        <div className="space-y-4">
            ...
            <div>
              ...
              <div className="glass-card divide-y divide-border">
                {pendingOccs.map((occ, i) => (
                  <motion.div key={occ.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="flex items-center justify-between p-4 gap-4 group">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <button onClick={() => handleToggleStatus(occ)} className="text-xs px-2 py-0.5 rounded-full font-medium bg-accent/20 text-accent hover:bg-primary/20 hover:text-primary transition-colors shrink-0" title="Marcar como pago">Pendente</button>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button onClick={() => setDeleteTarget(occ)} className="opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition-all"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </motion.div>
                ))}
              </div>
          ...
          {paidOccs.length > 0 && (
            <div>
              ...
              <div className="glass-card divide-y divide-border">
                {paidOccs.map((occ, i) => (
                  <motion.div key={occ.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="flex items-center justify-between p-4 gap-4 group opacity-60">
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <button onClick={() => handleToggleStatus(occ)} className="text-xs px-2 py-0.5 rounded-full font-medium bg-primary/20 text-primary hover:bg-accent/20 hover:text-accent transition-colors shrink-0" title="Desfazer pagamento">Pago</button>
                      <div className="min-w-0">
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Como você deseja excluir esta conta recorrente?</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter className="flex-col sm:flex-row gap-2">
            <AlertDialogCancel>Cancelar</AlertDialogCancel>
            <Button variant="outline" onClick={() => { if (deleteTarget) deleteOccurrenceMutation.mutate(deleteTarget.id); setDeleteTarget(null); }}>
              Excluir apenas este mês
            </Button>
            <AlertDialogAction className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => { if (deleteTarget) deleteMutation.mutate(deleteTarget.fixed_expense_id); setDeleteTarget(null); }}>
